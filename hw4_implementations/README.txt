...
- Change the `DESCRIPTION` file to include `Imports: igraph` (or however -- point is, make sure your `DESCRIPTION` file mentions the `igraph` package)
...
- Run the `hw4-demo_laptop.R` script. (This script assumes you have working `generate_partial_clique()` function
	...
	- It then applies all 25 implementations of `compute_maximal_partial_clique` on that `adj_mat` using `alpha=0.95`, where there's a 30 second time limit
...
- On Bayes
	- Log into Bayes (after using OpenSesame)
	...
	- Inside R:
		- Type in `devtools::install()`. (You might be asked to also install `igraph`, which might take a few minutes.)
